function semi_fnn_ff_func_d(data_name, kfolds, n_rules, n_agent, n_mixup, labeled_num, mu,rho_p,rho_s,beta,...
    gamma,eta, alpha)
    log_path = './log/';
    if exist(log_path,'dir')==0
       mkdir(log_path);
    end
    logfilename = sprintf('%s%s_d_ff_semifnn_r%d_l%.1f_g%.4f_mu%.4f_rho_p%.4f_rho_s%.4f_beta%.4f_eta%.4f_al%.4f.txt',...
...
%             rule_umg, n_mixup, gamma,alpha);
    ...
    save_dir = sprintf('./results/%s_d_ff_semifnn_d_r%d_l%.1f_g%.4f_mu%.4f_rho_p%.4f_rho_s%.4f_beta%.4f_eta%.4f_al%.4f.mat',...
