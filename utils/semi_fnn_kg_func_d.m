function semi_fnn_kg_func_d(data_name, kfolds, n_rules, n_agent, n_mixup, labeled_num, mu,rho_p,rho_s,beta,...
    gamma,eta, alpha)
    log_path = './log/';
    if exist(log_path,'dir')==0
       mkdir(log_path);
    end
    logfilename = sprintf('%s%s_d_kg_semifnn_r%d_l%.1f_g%.4f_mu%.4f_rho_p%.4f_rho_s%.4f_beta%.4f_eta%.4f_al%.4f.txt',...
...
%         end
        ...
        rule_g = rule_g.init_kmeans(squeeze(train_data.fea_d(1,:,:)), n_rules);
        [mu_optimal_g, ~] = Kmeans_ADMM(train_data.fea_d, n_rules, n_agent,rho_s);
%         rule_g = rule_g.update_kmeans(train_data.fea, mu_optimal_g);
        rule_g_arr = cell(1, n_agent);
        P_train_arr_g = cell(1, n_agent);
        ...
        for ii = 1:n_agent
            ...
            P_train_arr_g{ii} = ComputeP(squeeze(train_data.fea_d(ii,:,:)),rule_g_arr{ii});
            [~, w_train_arr_g(:,ii)] = FNN_solve(P_train_arr_g{ii}, train_data.gnd_d(ii,:)', mu);
        end
        [w_d_g, ~] = FNN_ADMM(train_data.gnd_d, n_agent, w_train_arr_g, P_train_arr_g, mu, rho_p);
        ...
        P_test_arr_g = cell(1, n_agent);
        
        for ii = 1:n_agent
            y_hat_train_g{ii} = P_train_arr_g{ii} * w_d_g;
            P_test_arr_g{ii} = ComputeP(test_data.fea,rule_g_arr{ii});
            y_hat_test_g{ii} = P_test_arr_g{ii} * w_d_g;
            if task=='C'
        ...
        rule_l = rule_l.init_kmeans(squeeze(train_data.fea_d_l(1,:,:)), n_rules);
        [mu_optimal_l, ~] = Kmeans_ADMM(train_data.fea_d_l, n_rules, n_agent,rho_s);
%         rule_g = rule_g.update_kmeans(train_data.fea, mu_optimal_g);
        rule_l_arr = cell(1, n_agent);
        P_train_arr_l = cell(1, n_agent);
        ...
        for ii = 1:n_agent
            ...
            P_train_arr_l{ii} = ComputeP(squeeze(train_data.fea_d_l(ii,:,:)),rule_l_arr{ii});
            [~, w_train_arr_l(:,ii)] = FNN_solve(P_train_arr_l{ii}, train_data.gnd_d_l(ii,:)', mu);
        end
        [w_d_l, ~] = FNN_ADMM(train_data.gnd_d_l, n_agent, w_train_arr_l, P_train_arr_l, mu, rho_p);
        ...
        P_test_arr_l = cell(1, n_agent);
        for ii = 1:n_agent
            y_hat_train_l{ii} = P_train_arr_l{ii} * w_d_l;
            P_test_arr_l{ii} = ComputeP(test_data.fea,rule_l_arr{ii});
            y_hat_test_l{ii} = P_test_arr_l{ii} * w_d_l;
            if task=='C'
...
%         rule_g = rule_g.update_kmeans(train_data.fea, mu_optimal_g);
        ...
        for ii = 1:n_agent
            ...
            P_train_arr_lg{ii} = ComputeP(squeeze(train_data.fea_d_l(ii,:,:)),rule_lg_arr{ii});
            [~, w_train_arr_lg(:,ii)] = FNN_solve(P_train_arr_lg{ii}, train_data.gnd_d_l(ii,:)', mu);
        end
        [w_d_lg, ~] = FNN_ADMM(train_data.gnd_d_l, n_agent, w_train_arr_lg, P_train_arr_lg, mu, rho_p);
        ...
        P_test_arr_lg = cell(1, n_agent);
        for ii = 1:n_agent
            y_hat_train_lg{ii} = P_train_arr_lg{ii} * w_d_lg;
            P_test_arr_lg{ii} = ComputeP(test_data.fea,rule_lg_arr{ii});
            y_hat_test_lg{ii} = P_test_arr_lg{ii} * w_d_lg;
            if task=='C'
        ...
        rule_ug = rule_g;
        [w_d_ug, ~] = graph_FNN_ADMM(train_data, n_agent, w_train_arr_lg, P_train_arr_lg, mu, rho_p,...
             P_train_arr_g, eta);
        ...
        P_train_arr_ug = cell(1, n_agent);
        y_hat_test_ug = cell(1, n_agent);
        P_test_arr_ug = cell(1, n_agent);
        ...
        for ii = 1:n_agent
            ...
            P_train_arr_ug{ii} = ComputeP(squeeze(train_data.fea_d_l(ii,:,:)),rule_ug_arr{ii});
            y_hat_train_ug{ii} = P_train_arr_ug{ii} * w_d_ug;
            P_test_arr_ug{ii} = ComputeP(test_data.fea,rule_ug_arr{ii});
            y_hat_test_ug{ii} = P_test_arr_ug{ii} * w_d_ug;
            if task=='C'
        ...
        rule_um = rule_g;
        [w_d_um, ~] = mixup_FNN_ADMM(train_data, n_agent, w_train_arr_l, P_train_arr_lg, mu, rho_p,...
        ...
        P_train_um = cell(1, n_agent);
        y_hat_test_um = cell(1, n_agent);
        P_test_um = cell(1, n_agent);
        ...
        for ii = 1:n_agent
            ...
            P_train_um{ii} = ComputeP(squeeze(train_data.fea_d_l(ii,:,:)),rule_um_arr{ii});
            y_hat_train_um{ii} = P_train_um{ii} * w_d_um;
            P_test_um{ii} = ComputeP(test_data.fea,rule_um_arr{ii});
            y_hat_test_um{ii} = P_test_um{ii} * w_d_um;
            if task=='C'
...
%         beta_umg = (mu*P_train_g'*L_hat*P_train_g + rho_p*B'*B + P_train_l'*P_train_l + mu*eye(p)) \ (P_train_l'*train_data.gnd_l);
        rule_umg = rule_g;
        
%         [w_d_umg, ~] = grapP_mixup_FNN_ADMM(train_data, n_agent, w_train_arr_lg,P_train_arr_g, P_train_arr_lg,...
%             rule_umg, n_mixup, mu,rho_p, gamma,eta,alpha);
        [w_d_umg, ~] = graph_mixup_FNN_ADMM(train_data, n_agent, w_train_arr_lg, P_train_arr_lg, mu, rho_p,...
            rule_umg, n_mixup, gamma,alpha,...
            P_train_arr_g, eta);
%         [w_d_um2, ~] = mixup_FNN_ADMM(train_data, n_agent, w_train_arr_lg, P_train_arr_lg, mu, rho_p,...
%             rule_umg, n_mixup, gamma,alpha);
        ...
        P_train_arr_umg = cell(1, n_agent);
        y_hat_test_umg = cell(1, n_agent);
        P_test_arr_umg = cell(1, n_agent);
        ...
        for ii = 1:n_agent
            ...
            P_train_arr_umg{ii} = ComputeP(squeeze(train_data.fea_d_l(ii,:,:)),rule_umg_arr{ii});
            y_hat_train_umg{ii} = P_train_arr_umg{ii} * w_d_umg;
            P_test_arr_umg{ii} = ComputeP(test_data.fea,rule_umg_arr{ii});
            y_hat_test_umg{ii} = P_test_arr_umg{ii} * w_d_umg;
            if task=='C'
    ...
    save_dir = sprintf('./results/%s_d_kg_semifnn_d_r%d_l%.1f_g%.4f_mu%.4f_rho_p%.4f_rho_s%.4f_beta%.4f_eta%.4f_al%.4f.mat',...
