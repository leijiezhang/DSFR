function semi_fnn_kg_func_c(data_name, kfolds, n_rules, n_mixup, labeled_num, rho, gamma, eta, alpha, beta)
    log_path = './log/';
    if exist(log_path,'dir')==0
       mkdir(log_path);
    end
    logfilename = sprintf('%s%s_c_kg_semifnn_r%d_l%.1f_la%.4f_eta%.4f_gamma%.4f_alpha%.4f_beta%.4f.txt',...
...
%         [eta_optimal_g, ~] = Kmeans_ADMM(train_data.fea_d, n_rules, n_agent);
        ...
        rule_g = rule_g.init_kmeans(train_data.fea, n_rules);
        H_train_g = ComputeH(train_data.fea,rule_g);
        [~, beta_g] = FNN_solve(H_train_g, train_data.gnd, rho);
        ...
        rule_l = rule_l.init_kmeans(train_data.fea_l, n_rules);
        H_train_l = ComputeH(train_data.fea_l,rule_l);
        [~, beta_l] = FNN_solve(H_train_l, train_data.gnd_l, rho);

        for j = 1:n_rules 
            rule_l.consq(j,:) = beta_l((j - 1)*(train_data.n_fea + 1) + 1: j*(train_data.n_fea + 1))';
        end 
        y_hat_train_l = H_train_l * beta_l;
        if task=='C'
        ...
        H_test_l = ComputeH(test_data.fea,rule_l);
        y_hat_test_l = H_test_l * beta_l;
        if task=='C'
        ...
        [~, beta_lg] = FNN_solve(H_train_lg, train_data.gnd_l, rho);
        ...
        [~, p] = size(H_train_l);
        Hinv = inv(eye(p)*rho  + H_train_lg' * H_train_lg + eta*H_train_g'*L_hat*H_train_g);
%       
...
%         [~, p] = size(H_train);
        ...
        beta_um = (gamma*B'*B + H_train_lg'*H_train_lg + rho*eye(p)) \ (H_train_lg'*train_data.gnd_l);
        ...
        y_hat_test_um = H_test_g * beta_um;
        if task=='C'
...
%         beta_umg = (eta*H_train_g'*L_hat*H_train_g + gamma*B'*B + H_train_l'*H_train_l + rho*eye(p)) \ (H_train_l'*train_data.gnd_l);
        
        beta_umg = (eta*H_train_g'*L_hat*H_train_g + gamma*B'*B + H_train_lg'*H_train_lg + rho*eye(p)) \ (H_train_lg'*train_data.gnd_l);
        for j = 1:n_rules 
            rule_u.consq(j,:) = beta_um((j - 1)*(train_data.n_fea + 1) + 1: j*(train_data.n_fea + 1))';
        end 
        ...
        y_hat_test_umg = H_test_g * beta_umg;
        if task=='C'
    ...
    save_dir = sprintf('./results/%s_c_kg_semifnn_r%d_l%.1f_la%.4f_eta%.4f_gamma%.4f_alpha%.4f_beta%.4f.mat',...
