function semi_fnn_fg_func_c_V1(data_name, kfolds, n_rules, n_mixup, labeled_num, rho, gamma, eta, alpha, beta)
    log_path = './log/';
    if exist(log_path,'dir')==0
       mkdir(log_path);
    end
    logfilename = sprintf('%s%s_c_fg_semifnn_r%d_l%.1f_rho%.4f_eta%.4f_gamma%.4f_alpha%.4f_beta%.4f.txt',...
...
%         [eta_optimal_g, ~] = Kmeans_ADMM(train_data.fea_d, n_rules, n_agent);
        ...
        rule_l = rule_l.update_fuzzyc(test_data.fea, rule_l.center, beta);
...
%         beta_umg = (eta*H_train_g'*L_hat*H_train_g + gamma*B'*B + H_train_l'*H_train_l + rho*eye(p)) \ (H_train_l'*train_data.gnd_l);
    ...
    save_dir = sprintf('./results/%s_c_fg_semifnn_r%d_l%.1f_la%.4f_eta%.4f_gamma%.4f_alpha%.4f_beta%.4f.mat',...
