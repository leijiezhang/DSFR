function [result_train_mean_g, result_train_std_g, result_test_mean_g, result_test_std_g, ...
    result_train_mean_lg, result_train_std_lg, result_test_mean_lg, result_test_std_lg]=...
        semi_fnn_fg_func_d_glg(data_name, kfolds, n_rules, n_agent, labeled_num, mu,rho_p,rho_s,beta,...
    fid)
...
%     alpha = 0.75;
    ...
    for i=1:kfolds
        ...
        for ii=1:n_agent
            ...
            gnd_d_tmp{ii} = train_data.gnd_d(i,:);
            fea_d_tmp{ii} = squeeze(train_data.fea_d(i,:,:));
...
%         tic;
        ...
        rule_g = rule_g.init_fuzzyc(train_data.fea_d{1}, n_rules);
...
%         tic;
        ...
        rule_lg = rule_lg.init_fuzzyc(train_data.fea_d{1}, n_rules);
        [mu_optimal_g, ~] = Fuzzycmeans_ADMM(train_data.fea_d, n_rules, n_agent,rho_s,beta);
        ...
        width_tmp_lg = zeros(size(rule_lg.width));
        for ii = 1:n_agent
